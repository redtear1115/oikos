...
- ⚠️ **`Avatar` API: viewer-relative `who` → absolute `memberRole`**（PR #243 #238）：`Avatar.tsx` 的 prop 從 `who: 'M' \| 'T'` 換成 `memberRole: 'a' \| 'b'`，`bg` 直接讀 `--ink` / `--accent` brand token；`MemberContext` 加 `whoToMemberRole(who, viewerIsA)` helper 給仍以 viewer 視角思考的 callsite 用。13 個 callsite 全部更新；SoloBanner 的佔位頭像顯式設為 `memberRole='b'`（缺席的伴侶概念上 = member_b）。
