...
- 有隱私 / 安全性質的條目（文案更正、資料遮罩、權限修補……）另外獨立成 `### Security` 小節，不要混進上面兩節
  - 門檻是「有沒有造成暴露」：這筆改動是否改變了資料實際的暴露面（誰看得到、送去哪裡、遮不遮罩），不是「形式上像不像文案更正」。反例：#1191 撤回「端對端加密」宣稱，是這個 repo 最具隱私色彩的文案更正，但沒有改變任何資料的暴露面，所以放在 1.5.14 的「使用者可見變化」；接續的 #1251 也分別在 1.5.15 的「使用者可見變化」與「技術變更」，兩次都不在 Security。
...
- **頁面標題與月份標題改用黑體（#1269、#1279）**
  ...
  技術：這些標題拿掉襯線字（`font-serif` class 與 inline 的 `var(--font-serif)`／`var(--font-fraunces)`）；DESIGN.md 記下決定的範圍與 `--font-serif` 刻意不補 fallback 的原因；RecurringRuleSheet 的 sticky 錯誤提示拿掉陰影。
- **停用中的按鈕透明度一致、油耗的「⋯」改成選單（#1358）**
  使用者：無法按的按鈕一律以同樣的淡度顯示；油耗紀錄頁右上角的「⋯」改成和其他頁一樣的下拉選單，點了先展開選單，不再直接跳出刪除確認；愛物頁小數字的「NT$」與金額之間不再空一格。
...
- **出遊的四個小調整（#1396）**
  使用者：折回主帳本的那筆紀錄副標顯示「出遊結算」；每次打開新增支出時都預設勾選所有參與者（包含後來才加入的人）；結束出遊的確認只留一個按鈕；沒有支出時的空狀態文案更簡潔。
  技術：以 `isOutingFoldNote` 比對四語的折回備註模板；SheetShell 新增 `headerSave` prop（預設 true）。
...
- **邀請連結改為 24 小時內有效（#1288）**
  ...
  技術：`expires_at` 以 DB 時鐘計算 `now() + 24h`，縮短外流連結可被使用的時間；acceptInvite 與 createInvite 的鎖順序統一，避免死結；`0067` 把既有邀請的期限改為 `created_at + 24h` 與原期限取較早者（須在程式上線後執行，不可逆）。
