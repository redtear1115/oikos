# docgrad scorecard — oikos @ 2026-07-13（第 3 輪後）

| 維度 | 星等 | 目標 | 主要失分點 |
|---|---|---|---|
| 完整性 | ★4 | ★4 | 達標。邊緣缺口：`supabase/functions/`、`scripts/` 無專屬文件 |
| 正確性 | ★4 | ★4 | 達標（第 3 輪 ★3→★4）。auth 分層敘述已對齊 code（v1.0.2 #494 改 getUser()）；ledger 8/8。附註：lib/supabase/server.ts 的 docstring 同樣 stale（docgrad 不動 code） |
