...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const t = dictionaries[locale].seo.migrate.andromoney
  return {
    title: t.title,
    description: t.description,
    alternates: buildAlternates('/migrate/andromoney', locale),
    openGraph: {
      title: t.title,
      description: t.ogDescription,
      url: localizedHref('/migrate/andromoney', locale),
...
export default async function MigrateAndromoney({ params }: { params: Params }) {
  const { locale: raw } = await params
  if (!isLocale(raw)) return null
  const locale: Locale = raw
  const t = dictionaries[locale].migrate
  const page = t.pages.andromoney
  const signInHref = localizedHref('/sign-in', locale)

  return (
    <div className="space-y-10 md:space-y-14">
      <MigrateBreadcrumbJsonLd locale={locale} source="andromoney" />
      <MigrateHowToJsonLd
        locale={locale}
        source="andromoney"
        name={page.heroTitle}
      ...
      <MigrateTool t={t} signInHref={signInHref} hint="andromoney" />
      ...
      <MigrateOtherSources locale={locale} currentSource="andromoney" copy={t.otherSources} />
