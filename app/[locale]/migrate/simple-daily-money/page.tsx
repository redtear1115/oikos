...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const t = dictionaries[locale].seo.migrate['simple-daily-money']
  return {
    title: t.title,
    description: t.description,
    alternates: buildAlternates('/migrate/simple-daily-money', locale),
    openGraph: {
      title: t.title,
      description: t.ogDescription,
      url: localizedHref('/migrate/simple-daily-money', locale),
      siteName: 'Futari',
      type: 'website',
...
export default async function MigrateSimpleDailyMoney({ params }: { params: Params }) {
  const { locale: raw } = await params
  if (!isLocale(raw)) return null
  const locale: Locale = raw
  const t = dictionaries[locale].migrate
  const page = t.pages['simple-daily-money']
  const signInHref = localizedHref('/sign-in', locale)

  return (
    <div className="space-y-10 md:space-y-14">
      <MigrateBreadcrumbJsonLd locale={locale} source="simple-daily-money" />
      <MigrateHowToJsonLd
        locale={locale}
      ...
      <MigrateChatgptWorkflow copy={t.chatgptWorkflow} source="simple-daily-money" />

      <MigrateTool t={t} signInHref={signInHref} hint="simple-daily-money" />
      ...
      <MigrateOtherSources locale={locale} currentSource="simple-daily-money" copy={t.otherSources} />
