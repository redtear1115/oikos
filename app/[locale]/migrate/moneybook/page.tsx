...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const t = dictionaries[locale].seo.migrate.moneybook
  return {
    title: t.title,
    description: t.description,
    alternates: buildAlternates('/migrate/moneybook', locale),
    openGraph: {
      title: t.title,
      description: t.ogDescription,
      url: localizedHref('/migrate/moneybook', locale),
      siteName: 'Futari',
      type: 'website',
...
export default async function MigrateMoneybook({ params }: { params: Params }) {
  const { locale: raw } = await params
  if (!isLocale(raw)) return null
  const locale: Locale = raw
  const t = dictionaries[locale].migrate
  const page = t.pages.moneybook
  const signInHref = localizedHref('/sign-in', locale)

  return (
    <div className="space-y-10 md:space-y-14">
      <MigrateBreadcrumbJsonLd locale={locale} source="moneybook" />
      <MigrateHowToJsonLd
        locale={locale}
        source="moneybook"
        name={page.heroTitle}
      ...
      <MigrateTool t={t} signInHref={signInHref} hint="moneybook" />
      ...
      <MigrateOtherSources locale={locale} currentSource="moneybook" copy={t.otherSources} />
