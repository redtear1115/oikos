...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: getHreflangAlternates('/terms', APP_URL),
    openGraph: {
      title: '服務條款 · Futari',
      description: 'Futari alpha 測試版本的服務條款與使用者注意事項。',
      type: 'article',
    ...
    <main className="min-h-screen px-6 py-12" style={{ background: 'var(--bg)' }}>
      <div className="max-w-md mx-auto">
        ...
        <div className="mt-12 flex gap-4 text-sm">
          <Link href={homeHref} className="underline" style={{ color: 'var(--ink-2)' }}>
            ← 回首頁
          </Link>
          <Link href={privacyHref} className="underline" style={{ color: 'var(--ink-2)' }}>
            隱私權政策
          </Link>
        </div>
      </div>
