...
import { countActiveRulesForAsset } from '@/lib/db/queries/recurringIncome'
...
export default async function AssetDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (asset.type === 'insurance') {
    ...
    if (framingGroup === 'savings' && insuranceDetailsData) {
      const [premiumStats, returnStats, returnByCat, premiumRows, returnRows, recurringRulesCount] = await Promise.all([
        ...
        countActiveRulesForAsset(asset.id, group.id),
      ])
