...
async function mirrorFont(font) {
  ...
  const header =
    ...
    ` * ${unique.length} woff2 files, ${(bytes / 1048576).toFixed(2)} MB, committed under public/fonts/${font.slug}/ and\n` +
    ` * bundled by Next into /_next/static/media/ (content-hashed, immutable-\n` +
    ` * cacheable) — these @font-face url()s are what triggers that bundling, so\n` +
    ` * public/fonts/ itself is never served at its own path.\n` +
