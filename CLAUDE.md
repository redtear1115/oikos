...
**Latest released: v1.5.3** — 完整版本歷史見 [CHANGELOG.md](CHANGELOG.md)
...
- **預設不派 subagent**：搜尋、實作、驗證都由主 session 依序自己做；只在 user 明確要求（「開 agent」「平行做」）或任務需要平行背景寫檔隔離時才委派。此規則優先於使用者全域 CLAUDE.md 的 Orchestration 委派政策。
