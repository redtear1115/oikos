...
**The Two-Grounds Rule.** `--bg` is where the couple works; `--bg-committed` is where the product speaks. Brand surfaces take the committed ground and the serif; app surfaces take the app ground and the sans. (§3 names a handful of first-run/connectivity screens — `/setup`, `/onboarding`, `/invite`, `/offline` — that keep the serif while staying on `--bg`; that is a §3 typography exception, not a second ground.) Never mix: a landing section on `--bg` looks unfinished, and a dashboard on `--bg-committed` looks like a marketing page wearing app chrome.
...
**The Serif-Speaks Rule.** Fraunces is the voice, not the workhorse. On brand surfaces it sets headings and narrative at full voice. Inside the app it is a deliberate exception, used for the moments that ask the couple to stop and look (the monthly review and quiz screens, the partner-left and solo-welcome cards, the leave-ledger and remove-partner flows, the confirm-modal title, the active-trip banner) and, more narrowly than "any name," for a name the moment you arrive at the thing it belongs to: a detail-page header (`AibutsuHeader`, `AssetHero`, `TripDetailClient`'s `h1`) or `ContextStrip` / `ActiveTripBanner`. The same name back in a list row, card, or switcher is sans — `AibutsuCard`, `CarHeroCard`, `InsuranceListItem`, `TripList`, `AssetSwitcher`, and `TripSelector` all set it in the sans face. Never set body lists, inputs, labels, buttons, or amounts in the serif — the one documented exception is the records donut's center amount (`MonthlyStatsPieChart`), kept deliberately serif as a headline moment rather than a figure to add up — and never the app's L1 page titles, its sub-page titles, or the records feed's month-section labels either; see the decided rule below. The sans does the work; the serif holds the feeling. *Test: if the serif text is something the user taps, types into, or adds up, it is wrong.*

Availability is never the argument, because the font loads everywhere: `app/layout.tsx` imports `app/fonts/fraunces.css` and puts `font-fraunces` on `<html>`, and the dashboard layout only swaps the *default* family to Noto Sans TC (`font-noto-tc` on its wrapper) without unloading anything. An earlier version of this rule said Fraunces was not loaded inside the dashboard. That was false, and it failed quietly: a design review followed it to a confident, well-cited P1 to strip the serif from intentional emotional moments (#1162). The comment on `.font-fraunces` in `fraunces.css` has since been corrected to say this plainly — it used to read "layouts opt subtrees in by class," which described an older setup and is what fed that P1; do not re-derive the old claim from memory of it. Fraunces also ships only Latin subsets, so in zh-TW / zh-CN / ja a serif heading draws its CJK glyphs from the platform fallback and reads as Fraunces only on Latin letters and digits.

**Decided (#1269, 2026-09-22): the app's L1 page titles (Dashboard, Records, Assets, Settings), its sub-page titles (Trust, Currency, Import — the Page tier above already said "page and sub-page headers"), and the records feed's month-section labels are sans**, matching the Page tier in the Hierarchy table above — the disagreement this section used to flag between the two is resolved in the Hierarchy table's favour. This includes the Dashboard's own L1 (`BrandHeader`), even though its content is the ledger name: that slot is the page title first, so it now reads in sans like every other L1, and the "names stay serif" allowance above no longer covers it. This decision is scoped to those titles; it does not touch the app's other existing serif — the emotional-moment list above, the detail-header/`ContextStrip`/`ActiveTripBanner` names also above, or the several spots that read as a "title" but were never in scope here and stay serif unchanged: `AssetsListClient`'s and `TripList`'s `SectionLabel`s, the records stats card's own `h2` (`MonthlyStatsView`), the donut-center amount (`MonthlyStatsPieChart`), `MonthSwitcher`, `DateRangeChip`, `NewFuelLog`'s form title, `coming-soon`'s `h1`, the dashboard empty-state title (`EmptyState`), the monthly-review banner's quote (`MonthlyReviewBanner`), the CSV-import success heading (`ImportResult`), and the policy-holder initial in an insurance row (`InsuranceListItem`).

`/setup`, `/onboarding`, `/invite`, and `/offline` are also brand surfaces for this rule (#1269, 2026-09-22) — first-run and connectivity screens outside the two-person daily flow, so they keep the serif voice. This is a typography call, not a ground call: all four render on the app ground (`--bg`), not the deepened `--bg-committed` that §1/§2's brand-surface ground list (landing, sign-in, migrate, terms, privacy) uses — Two-Grounds (§2) governs background color, and serif eligibility here is a separate axis that doesn't have to travel with it.

**The `--font-serif` token has no generic (`, serif`) fallback, left as-is on purpose for now (#1279, 2026-09-22).** `app/globals.css` defines `--font-serif: var(--font-fraunces)`, which itself resolves to `'Fraunces', 'Fraunces Fallback'` — the second name is a `local("Times New Roman")` face declared in `fraunces.css` with ascent/descent/line-gap/size-adjust overrides tuned to Fraunces' own metrics. Measured in headless Chromium (#1397 review): when the Fraunces webfont fails to load, text renders in `Fraunces Fallback` — a serif, not the inherited sans — and CSS font-family lookup never falls through to an inherited value once every listed family fails, so if `Fraunces Fallback` were also unavailable the browser would drop to its own UA-default serif (Times), not to sans either. A bare `, serif` at the end would only matter in that last, rare case, and it would not touch the actual CJK gap: Fraunces and its fallback are both Latin-only, so CJK glyphs already fall back per-glyph to whatever the platform substitutes regardless of a trailing generic keyword. Eight callsites across five files in the monthly review and quiz screens (`QuestionCard`, `QuizClient`, `WaitingScreen`, `PartnerQuizCard` ×3, `RevealScreen` ×2) inline `var(--font-fraunces), Georgia, serif` instead of the token. Their stack is a genuine two rungs longer than the token's — `Georgia`, then the generic keyword, after `Fraunces Fallback` — which is a real (if marginal) difference worth keeping, not a guard against a sans regression that doesn't actually happen either way; do not fold them into `--font-serif`.
