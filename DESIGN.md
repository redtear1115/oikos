...
**Decided (#1269, 2026-09-22): the app's L1 page titles (Dashboard, Records, Assets, Settings) and the records feed's month-section labels are sans**, matching the Page tier in the Hierarchy table above — the disagreement this section used to flag between the two is resolved in the Hierarchy table's favour. This includes the Dashboard's own L1 (`BrandHeader`), even though its content is the ledger name: that slot is the page title first, so it now reads in sans like every other L1, and the "names stay serif" allowance above no longer covers it. Serif inside the app is scoped to exactly the two things listed above — the emotional-moment screens/flows, and asset/trip names elsewhere — nothing else.
...
**The `--font-serif` token has no generic fallback, on purpose (#1279, 2026-09-22).** `app/globals.css` defines `--font-serif: var(--font-fraunces)` with nothing after it — no trailing `, serif`. That is deliberate, not an oversight: Fraunces ships Latin-only subsets, so a bare `serif` fallback would silently hand CJK glyphs to whatever generic serif face the platform happens to carry, which is not Fraunces and not a face this system chose. Rather than promise a CJK serif look it cannot deliver, the token falls through to the inherited sans on failure — the same face the rest of the app already reads in, so a dropped font degrades to "quieter," not to "wrong family." Six callsites across the monthly review and quiz screens (`QuestionCard`, `QuizClient`, `WaitingScreen`, `PartnerQuizCard`, `RevealScreen`) inline `var(--font-fraunces), Georgia, serif` instead of the token. That is not six copies of the same oversight: those screens lean Latin-heavy (numerals, short English labels) inside an already-serif emotional moment, so a Latin serif fallback reads better there than falling back to sans mid-quiz while Fraunces loads. Their stack is more complete than the token, not a workaround for the token's gap — do not fold them into `--font-serif`; that would be a downgrade.
