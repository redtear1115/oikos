...
**The Landing-Brand Exception (named, deliberate).** The register is `product` everywhere a user *works*, but `/` (the public landing) is the one brand surface — the page that decides whether a couple even tries the tool — and it is granted a **Committed** color budget the rest of the app is not. On landing *only*: the cream ground may give way to one committed warm section-break (an ember-soft band, not the cream → cream → cream march), the PhonePreview is promoted to the mobile hero, and a hand-drawn lamp illustration (the brand's literal symbol) anchors the hero. This is an exception scoped to `/`, never a license to loosen the One Ember Rule, the Pure-Black-and-White Ban, or the flat-by-default discipline anywhere a user works. "Committed" still means warm, not loud: a lamp left on for two people, not a spotlight. Decided in [#832](https://github.com/redtear1115/oikos/issues/832); the imagery craft is gated on delivery of the designer's lamp illustration, so this exception describes the agreed direction ahead of the pixels.
