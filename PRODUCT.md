...
This file is the strategy layer: who Futari is for, what it refuses to be, and what each surface is trying to do. Read it before writing copy, before judging a metric, and before any UI work. The other three: [README.md](README.md) for running and deploying the project, [CLAUDE.md](CLAUDE.md) for architecture and working conventions, and [DESIGN.md](DESIGN.md) for the visual system — that last one is generated by `/impeccable document` from `app/globals.css`, so never hand-edit it.
