---
status: shipped
shipped_in: v0.16.0（#222 — 愛物模板系統 v1，只有 `general` 模板）
note: 加入「物品 (item)」作為 TypePicker 的第七個選項，純文字 + 備註；舊 6 種 type 完全不動。將來擴充新模板（pet / child / plant / etc.）只需在 registry 加 key。
---

# 愛物模板系統

> 目標：為 TypePicker 加入一個「不需要任何後端行為」的輕量選項——使用者選「物品」、填名稱 + 備註，就結束。
> 範圍：Assets 新增 `template_key` enum + `template_fields` jsonb、`'item'` asset_type、TypePicker 新增「物品」選項、新的 `TemplateSheetBody` + `TemplateAssetDetailClient`、四語 i18n。
> 不在範圍：替舊 6 種 type 改建模板版本（car / child / pet / plant / house / insurance 完全不動）；新模板上線（如 `pet` 模板）需另外 issue。
...
| `pet` | `PetDetails` | `PetSheetBody` |
| `plant` | `PlantDetails` | `PlantSheetBody` |
| `insurance` | `InsuranceDetails` | `InsuranceSheetBody` |

這套模型在前六種 entity 上很合用——每個 type 都有自己的語意（FuelLog 雙寫、儲蓄險 framing、PII 加密、保費 cron）。

但 issue #222 觀察到：**有些東西使用者就是想記下來但不想要任何行為**——例如相機、單車、紀念物。每多開一個專屬 type 就要拉一條子表 + 一支 SheetBody，但其中根本沒有後端行為——只是純文字紀錄。

→ 開一條「模板路徑」，欄位 schema 在 code 裡宣告，值統一落到一個 jsonb，不開新子表、不寫新 cron。v1 範圍縮到最小：只有一個 `general` 模板，搭配 TypePicker 上的新「物品」選項。**舊 6 種 type 完全不動。**
...
`drizzle/0036_asset_templates.sql`：
...
`asset_template_key` enum 只有 `general`，可 append 不可移除（Postgres enum 限制）。將來新增模板：appendd enum value + 在 `lib/assetTemplates.ts` 加 key。
...
`validateTemplateFields(templateKey, raw)`：依模板 schema 驗證輸入。未知 key 默默丟棄、空值 → `null`、型別錯誤 → throw。雖然 v1 的 general 沒有欄位（永遠回 `{}`），驗證器內已實作 text / number / date 三種型別分支供未來模板使用。
...
**AssetSheet (`/AssetSheet/index.tsx`)** 是 router，邏輯與 v0.15 幾乎一致——TypePicker 顯示七個選項（前 4 個 primary + 「更多」展開 house / insurance / item），選到哪個就 dispatch 到對應的 SheetBody：
...
item      → TemplateSheetBody   ← 新增的這條
```

編輯模式同理：legacy 愛物根據 `initial.type` 派發；模板愛物根據 `templateKey != null` 走 'item' 分支。

**Detail 頁 (`/assets/[id]/page.tsx`)**：第一個分支檢查 `asset.templateKey != null`，命中走 `TemplateAssetDetailClient`；否則 fall through 到既有的 type-based 分流。

**愛物列表 (`AssetsListClient`)**：新增「物品」section，用 `items.filter(a => a.type === 'item')`。其餘 section（property / living / coverage）行為不變。
...
- [x] 可從 TypePicker 選「物品」建立 template-based 愛物
- [x] 編輯 / 顯示走 TemplateSheetBody + TemplateAssetDetailClient
- [x] 新增「物品」愛物不會觸發任何既有自動化（無新 cron、無 FuelLog 寫入、無 InsuranceDetails 寫入）
- [x] 既有 6 種 type 的新增 / 編輯流程完全沒有改變
...
- 模板擴充：以 `pet` / `child` / `plant` 為輕量模板加入登錄表（另開 issue）。
- 字段型別擴充：目前只有 text / number / date。如果之後要加 `enum`（下拉選單），改動點只有 `assetTemplates.ts` + 新版 TemplateSheetBody。
