...
| `?fCats=dining,transit` | 分類（多選，逗號分隔） | `?fCats=dining,transit` |
...
| DB query | `lib/db/queries/transactions.ts`、`lib/db/queries/incomes.ts` | 接 `ResolvedTxnFilter`（payer 解析成 user_id）+ `DateRange`，套到 SQL。`assetIds` 含 `__none__` 時拼成 `IS NULL OR IN(...)`。 |
...
- **settlements 在 split / cat / asset filter active 時整個 drop**：settlement 沒有 split_type / category / asset_id，套了也是零行——hide 比 leak 好。
- **income tab 遇到 expense-only dim → 整段空**：income 有自己的 category 詞彙；如果 user 點了 expense category 又切到 income tab，預期是「沒有」而不是「無視篩選」。
