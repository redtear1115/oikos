...
updates:
  ...
  - v0.16.2: v4 — 分類 sub-section 全選 chip + 每 chip 帶分類色點（#254）
related_specs: [transactions, stats, aibutsu]
related_issues: ["#50", "#165", "#223", "#235", "#254"]
---
...
## v4 增量（v0.16.2）— 分類 sub-section 全選 chip + 分類色點

純 UI 層 polish，不動 URL / 資料模型 / SQL —— 跟 v3 一樣是 sugar over 既有 `fCats` / `fIncCats`。

### Pain point

v0.15 開始 FilterSheet 雖然就有支出/收入分類 multi-select，但缺兩個 quality-of-life：

1. **沒有「全選」一鍵入口** — 想看「所有支出分類」（例如要排除收入但保留所有支出）得逐顆 tap。
2. **chip 不帶分類色** — 在一片同樣灰底的 chip 中視覺辨識度差；feed 列上的食/衣/住色票無法呼應到篩選面板，分類認知是斷的。

### 做法

- 每個分類 sub-section 開頭加一顆「全選」chip，沿用 v3 愛物 sub-section 完全相同的 toggle 語意：未全選時補齊全部 id；已全選時清空。Empty set 仍是「無 filter」語意，所以「全選 → 全選」兩 tap = 回到無篩選 baseline。
- 每顆分類 chip 左側加一個 6×6 圓點，使用 `Category.color` / `IncomeCategory.color`。Active state 不換色（仍是黑底白字 ink chip）—— 圓點在 active / inactive 兩種狀態都一直可見，確保分類身分色不會在被選後消失。
- 沿用既有的 `assetGroupSelectAll` i18n key（值都是「全選」），不開新字串。

### 為什麼 active state 不用分類色當底色

考量過 active = `category.color` 滿色背景（呼應 donut slice），但實作會讓 chip bar 在多選時變成彩虹條，視覺上反而比現在更亂；且部分分類（金融 `#607090` 等）跟白字對比不夠穩。圓點 + 黑底白字的方案：分類色一直在、但只佔小面積，active 對比清楚，多選時 chip bar 仍維持節制的同調色階。

### 跟現有 cross-kind cut 的銜接

「全選支出分類」會把全部 9 個 expense category id 寫進 `categories` set，size > 0 → `hidesSettlements` / `cutsIncome` 起作用：settlement 與 income 都會 cut 掉。這跟「只勾一個分類」的行為一致，不是 v4 新引入的副作用。使用者若要回到「show everything」，按重設或再 tap 一次全選清空即可。
...
- 分類 sub-section 全選 chip 行為一致；每顆分類 chip 左側帶分類色點
...
- **分類 chip active 用色塊背景**：彩虹條視覺更亂；改用圓點 + 黑底白字（v4）

## 後續

- **儲存為快速視圖**：「車子維修」之類命名快速視圖。需要新 schema（FilterPresets table）；不在現有版本範圍
