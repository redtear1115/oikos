...
|---|---|---|---|
| **v1.6.0** | 開局者（Futari 帳號用戶）開出遊、加朋友、代為記下每筆支出（選誰付、分給誰），看每人淨額與最少筆數轉帳建議、標記還款、結束出遊。結束時，兩位成員之間的相互欠額**實際折回**主帳本 balance | **名字**：`OutingParticipant` 沒有對應的 `Profile`，但從第一天就是獨立的 entity，之後可以接上帳號 | 先讓功能會動，驗證有沒有人要用。見 [solo-trip](solo-trip-design.md) Locked decision 1 的「為什麼分兩階段」 |
...
| 結束時回主帳本 | 2 筆 summary `CashTransactions`，進統計 | 只折夫妻相互欠額，寫一筆 `Settlement`：只動 balance、不進統計（理由見「折回兩人主帳本」） |
