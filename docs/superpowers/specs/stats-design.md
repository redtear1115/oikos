---
status: shipped
shipped_in: v0.14.0（PR #93 + 13 個後續 polish iterations：donut chart / collapse / mode-aware title / 收入 category breakdown 等），closes #22。Drill-down 於 v0.14.2 ship（PR #116，closes #102；v0.14.1 暫時 revert，v0.14.2 revert-the-revert）。
related_issues: "#22, #102"
note: v0.14.0 主題「沒有訊號的時候，也還看得見」的中段；#44 月度回顧 query 形狀沿用本 spec。
---
...
- ~~**Drill down 點 row → filtered transaction list**：nice-to-have，留 hook 不做~~ → v0.14.2 ship（PR #116, closes #102）。`lib/drill.ts` + `DrillFilterChip`，三個 tab 都通；同 row 再點一次清除。
...
- **Drill down**：點分類 / 愛物 row 跳到 records 既有 filter（filter dim = category / asset）— 實作見 #102 / PR #116（待 v0.14.2 deploy；v0.14.1 暫時 revert）
- **跨月對比**：本月 vs 上月差額條
