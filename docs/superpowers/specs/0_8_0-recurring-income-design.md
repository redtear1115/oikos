...
| 設定頁（規則列表 + 新增/編輯） | ⏳ Phase 1 | `app/(dashboard)/settings/recurring-income/` |
| Dashboard pending card stack | ⏳ Phase 1 | `app/(dashboard)/dashboard/_components/PendingIncomeStack.tsx` |
| IncomeSheet「改一下」prefill 整合 | ⏳ Phase 2 | 擴 `IncomeSheet.tsx` |
| 進帳 pill 未確認指示 dot | ⏳ Phase 2 | `ModeTogglePlaceholder.tsx` |
| Records 進帳 tab 設定入口 hint | ⏳ Phase 2 | `RecordsList.tsx` |
...
```sql
CREATE TABLE "RecurringIncomeRules" (
  id                  uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  group_id            uuid NOT NULL REFERENCES "OikosGroups"(id),
  recipient_id        uuid NOT NULL REFERENCES "Profiles"(id),
  amount              integer NOT NULL CHECK (amount > 0),
  category            text NOT NULL,                       -- IncomeCategoryId（薪 獎 期 賠 紅 退 副 其）
  source              text,                                -- 預設備註，如「公司 A 月薪」
  asset_id            uuid REFERENCES "Assets"(id),        -- 選填，常見=保單

  interval_months     integer NOT NULL DEFAULT 1 CHECK (interval_months > 0),
  day_of_month        integer NOT NULL CHECK (day_of_month BETWEEN 1 AND 31),
  starts_on           date NOT NULL,
  ends_on             date,                                -- 選填，固定期限合約用

  next_occurrence_at  date NOT NULL,                       -- 下次預定 occurredAt（cron 看這個）
  paused_at           timestamptz,                         -- 暫停（不影響 next_occurrence_at）
  deleted_at          timestamptz,
  created_at          timestamptz NOT NULL DEFAULT NOW()
);

CREATE INDEX "recurring_income_due_idx"
  ON "RecurringIncomeRules" (next_occurrence_at)
  WHERE deleted_at IS NULL AND paused_at IS NULL;

CREATE INDEX "recurring_income_group_idx"
...
```sql
CREATE TABLE "PendingIncomeOccurrences" (
  id                uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  group_id          uuid NOT NULL REFERENCES "OikosGroups"(id),     -- 反正規化以利 RLS / Realtime filter
  rule_id           uuid NOT NULL REFERENCES "RecurringIncomeRules"(id) ON DELETE CASCADE,  -- 只在 rule 硬刪時生效（pg_cron 1 年後物理刪）；軟刪由 server action 處理 active pendings
  period_start      date NOT NULL,                                  -- 該期錨日，等於該次 next_occurrence_at
  proposed_amount   integer NOT NULL CHECK (proposed_amount > 0),
  proposed_date     date NOT NULL,                                  -- 提示卡片建議的 occurredAt
  skipped_at        timestamptz,                                    -- 用戶選跳過時填入
  resolved_tx_id    uuid REFERENCES "IncomeTransactions"(id),       -- confirm 後指向產生的 tx（軟刪後仍可追溯）
  created_at        timestamptz NOT NULL DEFAULT NOW(),

  CONSTRAINT pending_unique_per_period UNIQUE (rule_id, period_start)
);

CREATE INDEX "pending_income_active_idx"
...
SELECT cron.schedule('generate-pending-income', '0 16 * * *', $$
  -- 對每個到期且 active 的規則，產生 pending（idempotent）
  ...
  -- 推進 next_occurrence_at（包含跳過 ends_on 之後的）
...
- `actions/recurringIncome.ts`：
  - `createRule`：算 next_occurrence_at = startsOn 之後第一個落在 day_of_month 的日期
  - `updateRule`：規則欄位變動時重算 next_occurrence_at（取 max(today, 新規則套到 startsOn 後第一個 anchor)）
  - `pauseRule`：只 set paused_at
  - `resumeRule`：clear paused_at；若 next_occurrence_at <= today，逐次 += interval_months 直到 > today（snap to future）
  - `softDeleteRule`：set deleted_at + 同 transaction DELETE active pendings
  - `confirmPending`：原子 INSERT IncomeTx + UPDATE pending.resolved_tx_id
  - `editAndConfirmPending`：同上但用編輯後 fields
  - `skipPending`：set pending.skipped_at
- `lib/db/queries/recurringIncome.ts`：`listActiveRules` / `listActivePendings`
- `app/(dashboard)/settings/recurring-income/page.tsx`：列表 + 新增/編輯 form
- `app/(dashboard)/dashboard/_components/PendingIncomeStack.tsx`：卡片堆疊
- Dashboard.tsx：進帳模式 mount PendingIncomeStack
- RealtimeProvider：訂閱 PendingIncomeOccurrences + RecurringIncomeRules，兩者 INSERT/UPDATE 觸發 router.refresh()

### Phase 2（v0.8.1 — 約 2-3 天）

**目標**：discovery + UX 細節。

- 進帳 pill dot 指示（支出模式時提示有 pending）
- Records 進帳 tab sticky header 加「設定定期進帳 →」入口
- IncomeSheet「改一下」prefill：sheet 接受可選 `pendingId` prop，submit 時走 `editAndConfirmPending`
- 跳過二次確認 toast（防誤觸）
- 規則列表頁的暫停/恢復 inline toggle
