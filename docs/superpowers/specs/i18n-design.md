...
> 狀態：基礎已 ship（PR #3 / #4 / #6 / #7 整併進 _Unreleased_），主要頁面接通。日期 helper、愛物詳情頁、設定子頁仍待補（見 [CLAUDE.md](../../../CLAUDE.md) Backlog）。
...
| 日期格式化 | **目前 hardcode 中文 helper**（`monthLabel` / `weekday` / `dateLabel`）— **backlog 待換 `Intl.DateTimeFormat(locale)`** | i18n 一期先把字典接通；日期 helper 換 Intl 是獨立工項，不阻擋字典發布 |
...
| 元件 | 路徑 | 角色 |
|---|---|---|
| Locale 常數 + helper | [lib/i18n/t.ts](../../../lib/i18n/t.ts) | `SUPPORTED_LOCALES` / `Locale` / `DEFAULT_LOCALE` / `LOCALE_COOKIE` / `isLocale()` / `getLocale()` / `getTranslations()` |
...
- **Date helpers 換 Intl**：`monthLabel` / `weekday` / `dateLabel` 目前硬寫中文，需改用 `Intl.DateTimeFormat(locale)` 讓月份標頭與日期顯示跟著語系走
...
- **`middleware.ts` 的 `Locale` type 只列 `'zh-TW' | 'en'`**：與 `lib/i18n/t.ts` 的 4 語不一致。實務上 `?lang=` query param 的入口很少用（主要切換路徑是 LanguageSwitcher 直接寫 cookie），但 `?lang=zh-CN` 或 `?lang=ja` 經 middleware 不會被持久化。修法：把 middleware 的 `isLocale` 改 import `lib/i18n/t.ts` 的版本
