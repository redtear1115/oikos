...
> 實測在 main checkout 跑 `cap sync` **不會**改動已 commit 的 `CapApp-SPM/Package.swift`（內容 byte-identical；worktree 裡則會被改寫成 worktree 深度的路徑，見 §K），
...
> 自 `CLAUDE.md` 搬入（2026-09-19，入口檔固定 token 稅）；內容逐字保留。只有做原生 build／在 worktree 做原生工作時才需要。
