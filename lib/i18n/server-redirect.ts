...
// sign-in URL.
//
// Why this exists: proxy's redirect-when-unauthed branch already picks
// the locale from cookie (see proxy.ts), but EXPLICIT redirects from
...
// destination. When the browser then GETs that URL, proxy sees an
...
// already locale-aware (e.g. `/ja/sign-in`), so proxy preserves rather
